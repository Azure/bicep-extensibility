...
/// Register with the factory overload of <c>AddHandlerBehavior</c> to pass the accepted versions:
/// <code>
/// .AddHandlerBehavior(_ => new ApiVersionValidationBehavior("2024-01-01"))
/// </code>
/// </summary>
    ...
    private ErrorResponse? Validate(string? apiVersion) =>
        apiVersion is null || !this.acceptedApiVersions.Contains(apiVersion)
            ? new ErrorResponse(new Error
            {
                ...
                Target = JsonPointer.Parse("/apiVersion"),
            })
            : null;
}
